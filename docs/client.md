...
The **no-arg constructor** is what you'll use in production. It resolves the token and settings from the [configuration cascade](./settings.md#configuration-cascade) and fires a single `GET /user/` request to validate the token before returning.
